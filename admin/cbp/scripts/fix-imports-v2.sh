...
fix_imports() {
    ...
    sed -E '
        ...
        # Fix incorrect relative paths starting with /../../
        s/\/\.\.\//\.\.\//g;
        
        # Convert @cbp-admin/ to @/ for consistency
        s/@cbp-admin\//@\//g;
        
        # Fix paths with duplicate type names
        s/\/types\/([a-zA-Z0-9_-]+)\.types\/types\/\1\.types/\/types\/\1.types/g;
        
        # Fix paths with duplicate service names
        s/\/services\/([a-zA-Z0-9_-]+)\/services\/\1/\/services\/\1/g;
        
        # Fix paths with duplicate component names
        s/\/components\/([a-zA-Z0-9_-]+)\/components\/\1/\/components\/\1/g;
        
        # Fix paths with duplicate interface names
