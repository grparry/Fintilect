  ...
  http.get('*/api/v1/payment/pending/summary', () => {
    const summary: PendingPaymentSummary = {
      ...
      byStatus: {
        ...
        [PaymentStatus.EXPIRED]: mockPendingPayments.filter(p => p.status === PaymentStatus.EXPIRED).length,
        [PaymentStatus.PENDING_APPROVAL]: mockPendingPayments.filter(p => p.status === PaymentStatus.PENDING_APPROVAL).length,
        [PaymentStatus.DRAFT]: mockPendingPayments.filter(p => p.status === PaymentStatus.DRAFT).length,
        [PaymentStatus.SUBMITTED]: mockPendingPayments.filter(p => p.status === PaymentStatus.SUBMITTED).length,
        [PaymentStatus.SCHEDULED]: mockPendingPayments.filter(p => p.status === PaymentStatus.SCHEDULED).length,
        [PaymentStatus.RETURNED]: mockPendingPayments.filter(p => p.status === PaymentStatus.RETURNED).length,
        [PaymentStatus.STOP_PAYMENT]: mockPendingPayments.filter(p => p.status === PaymentStatus.STOP_PAYMENT).length,
        [PaymentStatus.REVERSED]: mockPendingPayments.filter(p => p.status === PaymentStatus.REVERSED).length,
        [PaymentStatus.REFUNDED]: mockPendingPayments.filter(p => p.status === PaymentStatus.REFUNDED).length,
        [PaymentStatus.RESENT]: mockPendingPayments.filter(p => p.status === PaymentStatus.RESENT).length,
        [PaymentStatus.REINITIATED]: mockPendingPayments.filter(p => p.status === PaymentStatus.REINITIATED).length,
        [PaymentStatus.PENDING_REVERSAL]: mockPendingPayments.filter(p => p.status === PaymentStatus.PENDING_REVERSAL).length,
        [PaymentStatus.PENDING_REFUND]: mockPendingPayments.filter(p => p.status === PaymentStatus.PENDING_REFUND).length,
        [PaymentStatus.PENDING_RETURN]: mockPendingPayments.filter(p => p.status === PaymentStatus.PENDING_RETURN).length,
        [PaymentStatus.PENDING_STOP_PAYMENT]: mockPendingPayments.filter(p => p.status === PaymentStatus.PENDING_STOP_PAYMENT).length,
        [PaymentStatus.PENDING_RESEND]: mockPendingPayments.filter(p => p.status === PaymentStatus.PENDING_RESEND).length,
        [PaymentStatus.PENDING_REINITIATE]: mockPendingPayments.filter(p => p.status === PaymentStatus.PENDING_REINITIATE).length
      },
      byPriority: {
