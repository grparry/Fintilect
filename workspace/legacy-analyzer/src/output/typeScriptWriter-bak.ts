...
export class TypeScriptWriter {
    ...
    private mapCSharpTypeToTypeScript(csharpType: string): string {
        ...
        const nullableMatch = csharpType.match(/^(.+)\?$/);
        if (nullableMatch) {
            const [_, baseType] = nullableMatch;
            return `${this.mapCSharpTypeToTypeScript(baseType)} | null`;
        }

        // Check for Nullable<T> syntax
        const nullableGenericMatch = csharpType.match(/^Nullable<(.+)>$/);
        if (nullableGenericMatch) {
            const [_, baseType] = nullableGenericMatch;
            return `${this.mapCSharpTypeToTypeScript(baseType)} | null`;
        }
        ...
        if (arrayMatch) {
            ...
            // Map the element type to handle primitive types
            const mappedElementType = this.mapCSharpTypeToTypeScript(elementType);
            return `${mappedElementType}[]`;
        }

        // Check for generic types like List<T>
        const genericMatch = csharpType.match(/^(List|IList|IEnumerable|ICollection|HashSet|ApiResponse)<(.+)>$/);
        if (genericMatch) {
            const [_, genericType, innerType] = genericMatch;
            // Map common C# collection types to TypeScript array
            if (['List', 'IList', 'IEnumerable', 'ICollection', 'HashSet'].includes(genericType)) {
                // First map the inner type to handle primitive types
                const mappedInnerType = this.mapCSharpTypeToTypeScript(innerType.trim());
                return `${mappedInnerType}[]`;
            }
            // For other generic types, keep the original name
            return `${genericType}<${this.mapCSharpTypeToTypeScript(innerType.trim())}>`;
        }

        // Handle dictionary types
        const dictionaryMatch = csharpType.match(/^(I?Dictionary)<([^,]+),\s*([^>]+)>/);
        if (dictionaryMatch) {
            const [_, __, keyTypeRaw, valueTypeRaw] = dictionaryMatch;
            const keyType = this.mapCSharpTypeToTypeScript(keyTypeRaw.trim());
            const valueType = this.mapCSharpTypeToTypeScript(valueTypeRaw.trim());
    ...
    private getTypeForValue(value: any, propertyName?: string): string {
        if (value === null) {
            // Infer type from property name if available
            if (propertyName) {
                if (propertyName.toLowerCase().includes('string')) return 'null | string';
                if (propertyName.toLowerCase().includes('object')) return 'null | object';
                if (propertyName.toLowerCase().includes('array')) return 'null | any[]';
            }
            return 'null';
        }
    ...
    private generateObjectInterface(obj: any): string {
        ...
        const properties = Object.entries(obj).map(([key, value]) => {
            const type = this.getTypeForValue(value, key);
    ...
    private generateTypeFromValue(value: any, indent: string = '', name?: string): string {
        ...
        const interfaceContent = `{\n${Object.entries(value).map(([key, val]) => {
            const type = this.getTypeForValue(val, key);
    ...
    private formatPropertyName(key: string): string {
        // Handle special characters in property names by wrapping them in quotes
        // Note: $ is treated as a special character that needs quoting
